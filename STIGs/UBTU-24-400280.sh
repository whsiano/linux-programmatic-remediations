...
#
# .SYNOPSIS
#     Remediates UBTU-24-400280: Ubuntu 24.04 LTS must enforce password
...
#     Documentation   : https://stigaview.com/products/ubuntu2404/v1r5/UBTU-24-400280/
#
# .TESTED ON
#     Date(s) Tested  :
#     Tested By       :
...
#
# .USAGE
#     Requires root. Requires the libpam-pwquality package.
#
#     NOTE: This affects password CHANGES going forward. Existing passwords
#     are unaffected until the next time they are changed. The setting is only
#     enforced if pam_pwquality.so is present in /etc/pam.d/common-password.
#
#     Example syntax:
#     $ sudo ./UBTU-24-400280.sh
#
...
# Count ACTIVE definitions only. Anchoring at the start of the line keeps the
# stock commented example (e.g. '# dcredit = 0') out of the match, so it is
...
if [[ "${COUNT}" -eq 1 && "${FOUND}" == "${VALUE}" ]]; then
    echo "[+] UBTU-24-400280 remediated: ${PARAM}=${FOUND}"
